...
EVAL_ITERS     = 20        # 评估 batch 数（从 50 降到 20，省时间）
LR             = 3e-4      # 峰值学习率
...
def get_batch(data, block_size, batch_size, device):
    ...
    向量化采样：用 numpy 一次组好整个 (B, T) 矩阵，避免 Python 循环。
        - 用 numpy 的 fancy indexing 一次性切 batch_size 条样本
        - uint16 → int64 一次性转换
        - pin_memory + non_blocking 把 CPU→GPU 拷贝异步化
    """
    n = len(data) - block_size - 1
    ix = np.random.randint(0, n, size=batch_size)
    # 构造 (B, T) 索引矩阵：第 b 行是 [ix[b], ix[b]+1, ..., ix[b]+T-1]
    ...
    for name, data in splits.items():
        ...
        for k in range(EVAL_ITERS):
            ...
            if USE_AMP:
                ...
                _, loss = model(x, y)
            losses[k] = loss.detach()           # 留在 GPU，避免每步同步
        out[name] = losses.mean().item()        # 只在最后同步一次
    model.train()
    return out
